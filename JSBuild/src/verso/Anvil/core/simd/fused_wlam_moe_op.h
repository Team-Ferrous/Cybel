// highnoon/_native/ops/fused_wlam_moe_op.h
 ...
 *
 * See HIGHNOON_F1_OPTIMIZATION_ROADMAP.md Phase 5.2.
 */

#ifndef HIGHNOON_NATIVE_OPS_FUSED_WLAM_MOE_OP_H_
#define HIGHNOON_NATIVE_OPS_FUSED_WLAM_MOE_OP_H_
...
inline void holographic_routing(
    ...
    highnoon::ops::fft_forward_real(query, query_fft, hd_dim);
    
    for (int e = 0; e < num_experts; ++e) {
        const float* key = expert_keys + e * hd_dim;
        
        // FFT of expert key
        highnoon::ops::fft_forward_real(key, key_fft, hd_dim);
...
inline void FusedWLAMMoEForward(
    ...
    if (config.use_streaming_buffer) {
        streaming_buffer = hsmn::ops::GetTensorStreamPool().Acquire(
    ...
    if (config.use_streaming_buffer && streaming_buffer != fallback.data()) {
        hsmn::ops::GetTensorStreamPool().Release(streaming_buffer);
    }
}
...
#endif  // HIGHNOON_NATIVE_OPS_FUSED_WLAM_MOE_OP_H_
