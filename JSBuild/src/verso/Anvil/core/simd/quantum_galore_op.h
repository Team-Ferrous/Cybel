// highnoon/_native/ops/quantum_galore_op.h
...
#ifndef HIGHNOON_NATIVE_OPS_QUANTUM_GALORE_OP_H_
#define HIGHNOON_NATIVE_OPS_QUANTUM_GALORE_OP_H_
    ...
    if (project_rows) {
        ...
        for (int r = 0; r < rank; ++r) {
            for (int c = 0; c < cols; ++c) {
                ...
                for (int i = 0; i < rows; ++i) {
                    ...
                    if (i + 4 < rows) {
                        hsmn::ops::PrefetchT0(gradient + (i + 4) * cols + c);
                        hsmn::ops::PrefetchT0(rotation_matrix + r * rows + i + 4);
                    }
        ...
        for (int i = 0; i < rows; ++i) {
            for (int r = 0; r < rank; ++r) {
                ...
                if (i + 1 < rows) {
                    hsmn::ops::PrefetchT0(gradient + (i + 1) * cols);
                }
                if (r + 1 < rank) {
                    hsmn::ops::PrefetchT1(rotation_matrix + (r + 1) * cols);
                }
...
#endif  // HIGHNOON_NATIVE_OPS_QUANTUM_GALORE_OP_H_
