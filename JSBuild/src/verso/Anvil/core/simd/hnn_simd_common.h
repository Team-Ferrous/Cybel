// highnoon/_native/ops/hnn_simd_common.h
 ...
 *
 * All functions use the simd_ prefix and are in namespace highnoon::ops.
 * Thread-safe: all functions are reentrant with no shared state.
 *
 * Precision: float32 only (maintains O(n) linear complexity)
 */

#ifndef HIGHNOON_NATIVE_OPS_HNN_SIMD_COMMON_H_
#define HIGHNOON_NATIVE_OPS_HNN_SIMD_COMMON_H_
...
struct PathScratchPool {
    ...
    float* buffers[kMaxSlots] = {nullptr};
    size_t capacities[kMaxSlots] = {0};
    
    /**
     * @brief Get a scratch buffer of at least the requested size from the specified slot.
     * 
     * @param size_floats Number of floats needed
     * @param slot Slot index (0-7), allows strictly distinct buffers in same thread
     * @return Pointer to aligned scratch buffer (valid until next get() on same slot)
     */
    float* get(size_t size_floats, int slot = 0) {
        if (slot < 0 || slot >= kMaxSlots) return nullptr;

        if (size_floats > capacities[slot]) {
            // Double capacity to amortize reallocations
            size_t new_capacity = std::max(size_floats, capacities[slot] * 2);
            // Minimum 64KB to avoid frequent small reallocations
            new_capacity = std::max(new_capacity, size_t(16384));
            
            if (buffers[slot] != nullptr) {
                aligned_free_simd(buffers[slot]);
            }
            buffers[slot] = static_cast<float*>(aligned_alloc_simd(new_capacity * sizeof(float)));
            capacities[slot] = (buffers[slot] != nullptr) ? new_capacity : 0;
        }
        return buffers[slot];
    }
    
    /**
     * @brief Clear the pool, freeing all memory in all slots.
     */
    void clear() {
        for (int i = 0; i < kMaxSlots; ++i) {
            if (buffers[i] != nullptr) {
                aligned_free_simd(buffers[i]);
                buffers[i] = nullptr;
                capacities[i] = 0;
            }
        }
...
inline void simd_sigmoid_inplace(float* data, int64_t size) {
...
#if defined(__AVX512F__)
    // Use numerically stable sigmoid with clamping
    for (; i + 16 <= size; i += 16) {
        __m512 x = _mm512_loadu_ps(&data[i]);
        ...
        _mm512_storeu_ps(&data[i], x);  // Store clamped values
    }
    
    // Apply sigmoid in scalar for numerical stability
    for (int64_t j = 0; j < i; ++j) {
        float x = data[j];
        if (x >= 0.0f) {
            float exp_neg = std::exp(-x);
            data[j] = 1.0f / (1.0f + exp_neg);
        } else {
            float exp_x = std::exp(x);
            data[j] = exp_x / (1.0f + exp_x);
        }
    }
#elif defined(__AVX2__)
    // Use numerically stable sigmoid: 1/(1+exp(-x)) for x>=0, exp(x)/(1+exp(x)) for x<0
    const __m256 one = _mm256_set1_ps(1.0f);
    ...
    for (; i + 8 <= size; i += 8) {
        ...
        // Clamp input to avoid overflow/underflow in exp
        // sigmoid(-88) ≈ 0, sigmoid(88) ≈ 1 for float32
        __m256 x_clamped = _mm256_max_ps(x, _mm256_set1_ps(-88.0f));
        x_clamped = _mm256_min_ps(x_clamped, _mm256_set1_ps(88.0f));
        
        // For all elements, compute exp(-abs(x)) to avoid overflow
        __m256 neg_abs_x = _mm256_mul_ps(_mm256_andnot_ps(_mm256_set1_ps(-0.0f), x_clamped), _mm256_set1_ps(-1.0f));
        
        // Compute exp(-|x|) using std::exp in scalar loop (SIMD exp can be added later)
        // For now, fall through to scalar for stability
        _mm256_storeu_ps(&data[i], x_clamped);  // Store clamped values temporarily
    }
    
    // Apply exp and sigmoid in scalar for AVX2 (numerically stable)
    for (int64_t j = 0; j < i; ++j) {
        float x = data[j];
        if (x >= 0.0f) {
            float exp_neg = std::exp(-x);
...
#endif  // HIGHNOON_NATIVE_OPS_HNN_SIMD_COMMON_H_
