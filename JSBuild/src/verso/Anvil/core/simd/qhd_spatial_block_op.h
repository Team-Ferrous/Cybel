// highnoon/_native/ops/qhd_spatial_block_op.h
...
#ifndef HIGHNOON_NATIVE_OPS_QHD_SPATIAL_BLOCK_OP_H_
#define HIGHNOON_NATIVE_OPS_QHD_SPATIAL_BLOCK_OP_H_
...
using ::highnoon::ops::fft_butterfly;
...
// are defined at global scope in hnn_simd_common.h (before namespace highnoon::ops)
...
//
// See HIGHNOON_V2_PERFORMANCE_ANALYSIS.md Section 13.2 (Phase P1)
// =============================================================================
...
inline void ssm_freq_update_superposition(
    ...
    for (int k = 0; k < K; ++k) {
        ...
        const float* mask_k = (config.use_frequency_stratification) ? 
            (highnoon::ops::FrequencyMaskCache::instance().get_masks(config) + k * hd_dim) : nullptr;
        ...
        highnoon::ops::simd_exp_inplace(exp_workspace.data(), state_size);
...
inline void born_rule_collapse(
    ...
    highnoon::ops::simd_log_inplace(log_probs.data(), K);
...
inline void QHDSpatialForward(
    ...
    highnoon::ops::UnifiedQuantumBus::instance().get_born_amplitudes(warm_start_probs.data(), batch_size, K);
    ...
    highnoon::ops::UnifiedQuantumBus::instance().set_born_amplitudes(final_batch_probs.data(), batch_size, K);
}
...
inline void QHDSpatialForwardStreaming(
    ...
    if (using_pool) {
        ...
        output_buffer = hsmn::ops::GetTensorStreamPool().Acquire(output_size, "QHDSpatialForward");
    ...
    if (using_pool) {
        hsmn::ops::GetTensorStreamPool().Handoff(output_buffer, consumer_hint);
    }
}
...
#endif  // HIGHNOON_NATIVE_OPS_QHD_SPATIAL_BLOCK_OP_H_
