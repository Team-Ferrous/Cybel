// highnoon/_native/ops/fused_text_tokenizer_op.h
...
#ifndef HIGHNOON_NATIVE_OPS_FUSED_TEXT_TOKENIZER_OP_H_
#define HIGHNOON_NATIVE_OPS_FUSED_TEXT_TOKENIZER_OP_H_
    ...
    auto process_range = [&](int start, int end) {
        for (int i = start; i < end; ++i) {
            ...
            if (len > max_length) {
                len = max_length;
                if (add_special_tokens) {
                    out_ptr[max_length - 1] = TEXT_TOK_EOS_ID;  // Ensure EOS
                }
            }

            output_lengths[i] = len;
...
#endif  // HIGHNOON_NATIVE_OPS_FUSED_TEXT_TOKENIZER_OP_H_
