// highnoon/_native/ops/fused_hd_hierarchical_block_op.h
...
#ifndef HIGHNOON_NATIVE_OPS_FUSED_HD_HIERARCHICAL_BLOCK_OP_H_
#define HIGHNOON_NATIVE_OPS_FUSED_HD_HIERARCHICAL_BLOCK_OP_H_
...
inline void hier_add_scaled(
    ...
    hsmn::simd::simd_add_scaled(a, b, scale, c, size);
}
...
inline void hier_ema_blend(
    ...
    hsmn::simd::simd_ema_blend(memory, aggregated, alpha, output, size);
}
...
inline float cosine_similarity(const float* a, const float* b, int64_t dim) {
    return hsmn::simd::simd_cosine_similarity(a, b, dim);
}
...
#endif  // HIGHNOON_NATIVE_OPS_FUSED_HD_HIERARCHICAL_BLOCK_OP_H_
