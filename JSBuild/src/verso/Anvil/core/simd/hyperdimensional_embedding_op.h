// highnoon/_native/ops/hyperdimensional_embedding_op.h
...
#ifndef HIGHNOON_NATIVE_OPS_HYPERDIMENSIONAL_EMBEDDING_OP_H_
#define HIGHNOON_NATIVE_OPS_HYPERDIMENSIONAL_EMBEDDING_OP_H_
...
inline void CTQWSpreadGrad(const float* grad_output_emb, const float* original_input_emb, float* grad_input_emb, int batch, int dim, int steps);
...
inline void CTQWSpreadGrad(const float* grad_output_emb, const float* original_input_emb, float* grad_input_emb, int batch, int dim, int steps) {
    std::vector<float> current_grad_out(dim); // Buffer to hold gradients for current step
    std::vector<float> accumulated_grad(dim); // Accumulates gradients for input of current batch

    for (int b = 0; b < batch; ++b) {
        // Pointers for current batch
        const float* incoming_grad_ptr = grad_output_emb + b * dim;
        float* output_grad_ptr = grad_input_emb + b * dim;

        // Initialize with the incoming gradient for this batch
        std::copy_n(incoming_grad_ptr, dim, current_grad_out.data());
        std::fill(accumulated_grad.begin(), accumulated_grad.end(), 0.0f); // Zero out accumulator

        for (int t = 0; t < steps; ++t) {
            std::vector<float> next_grad_in_step(dim); // Stores gradients propagated backward one step

            for (int d = 0; d < dim; ++d) {
                int prev = (d - 1 + dim) % dim;
                int next = (d + 1) % dim;

                // Propagate gradient backwards through one diffusion step
                // gradX_d = 0.5 * gradY_d + 0.25 * gradY_{d-1} + 0.25 * gradY_{d+1}
                next_grad_in_step[d] = 0.5f * current_grad_out[d]
                                    + 0.25f * current_grad_out[prev]
                                    + 0.25f * current_grad_out[next];
            }
            // Update current_grad_out for next iteration (as if going backwards in time)
            std::copy(next_grad_in_step.begin(), next_grad_in_step.end(), current_grad_out.data());
        }
        // After 'steps' backward iterations, current_grad_out contains the accumulated gradient for the original input of this batch
        std::copy_n(current_grad_out.data(), dim, output_grad_ptr);
    }
}
