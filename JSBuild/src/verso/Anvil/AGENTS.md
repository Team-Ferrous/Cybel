...
This is the repository for the **Anvil**, a sophisticated, multi-agent system for code understanding and generation. The agent is designed with a focus on enterprise-level features, including:

*   **Multi-Agent Architecture:** A `MasterAgent` orchestrates `SubAgent`s to perform complex tasks.
*   **Semantic Code Intelligence:** The "Saguaro" component provides deep code understanding through semantic search, impact analysis, and code slicing.
*   **Multiple Execution Loops:** The agent can operate in different modes, from a simple conversational chat to a fully autonomous agent that can plan and execute complex tasks.
*   **Command-Line Interface:** A rich REPL provides a wide range of commands for interacting with the agent.
*   **Extensive Configuration:** The agent's behavior can be fine-tuned through a comprehensive set of configuration options.
*   **Performance Optimizations:** The agent includes several performance optimizations, including GPU acceleration and advanced caching techniques.

## Building and Running

### Building

The project includes a native component that needs to be built. The build process is automatically triggered when you install the project in editable mode:

```bash
pip install -e .
```

This will run the `core/native/build_native.sh` script to build the native extension.

### Running the REPL

The primary way to interact with the Anvil is through its REPL (Read-Eval-Print Loop). To start the REPL, run the following command:

```bash
granite
```

This will launch the `AgentREPL` and provide you with a `anvil>` prompt.

### Running a Mission

You can also run a "mission" directly from the command line by passing an objective to `main.py`:
...
*   **Configuration:** The agent is configured through the `config/settings.py` file.
*   **Saguaro:** The Saguaro tool is a key component of the agent's code intelligence capabilities. It is used for semantic search, impact analysis, and code slicing. When performing analysis or codebase investigation, the agent natively uses Saguaro tool calls within the project's virtual environment.
*   **Agentic Thinking:** The agent uses a "Thinking Protocol" to reason about problems and plan its actions. This is visible in the `<thinking>` blocks in the agent's output.
*   **Artifacts:** The agent saves its plans, tasks, and other artifacts to the `.anvil/` directory.
...
- `view_file` (for exploration) → Use `saguaro agent skeleton` then `slice`
...
3. Need to read a specific function/class?
   → saguaro agent slice ClassName.method --depth 2
...
saguaro query "user authentication logic" --k 5

# ✅ CORRECT: Get actual code content with metadata (Structured Retrieval)
saguaro query "user authentication logic" --json
...
```bash
# 1. Apply Patch
saguaro agent patch src/core.py patch.json

# 2. Verify & Commit
saguaro agent verify <sandbox_id>
saguaro agent commit <sandbox_id>
```
...
2. **Check if index exists**: `saguaro health`
3. **Rebuild if needed**: `saguaro index --path .`
4. **Only then** use fallback tools, with explicit justification

```bash
# Example error handling workflow
saguaro query "auth logic" --k 5
...
```bash
saguaro agent skeleton /path/to/large_file.py
# Identify the function you need, then:
saguaro agent slice TargetFunction --depth 2
```
...
saguaro workset create --desc "Implement login flow" --files "src/auth.py,tests/test_auth.py"

# List active worksets
saguaro workset list
```
...
saguaro refactor plan --symbol "ClassName"
```
...
saguaro impact --path saguaro/core.py

# Find unreachable code candidates
saguaro deadcode

# Detect isolated unreachable feature islands
...
saguaro knowledge add --category invariant --key "auth_model" --value "RBAC"
...
saguaro agent run planner --task "Refactor Auth"
...
saguaro memory --list
saguaro memory --write "auth_status" "migrated"
```
...
saguaro scribe "Refactor auth logic" --file src/auth.py --out patch.json
```
...
saguaro agent verify a1b2c3d4

# 3. Predict Impact Risk
saguaro agent impact a1b2c3d4

# 4. Commit to Disk (Triggers Micro-Indexing)
saguaro agent commit a1b2c3d4
```
...
saguaro simulate volatility

# Predict Regressions for changed files
saguaro simulate regression --files "src/auth.py"
```
...
```bash
# Test Intent Routing
saguaro route "fix the login bug"
...
## Troubleshooting
- **Memory/Segfaults**: If `saguaro index` crashes, ensure `native saguaro core` is built with the correct TensorFlow ABI flags.
