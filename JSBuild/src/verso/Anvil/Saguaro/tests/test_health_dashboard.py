...
import saguaro.health as health_module
...
def mock_saguaro_dir(tmp_path):
    ...
    config = {
        ...
        "last_index": {"peak_memory_mb": 150.5, "files": 10, "entities": 100},
    }
...
@pytest.fixture
def native_compute_stub(monkeypatch):
    payload = {
        "status": "ready",
...
def test_health_report_generation(mock_saguaro_dir, native_compute_stub):
    ...
    assert "native_compute" in report
    ...
    assert report["native_compute"]["required_simd"] == "avx2"
    assert report["state_journal"]["count"] == 0
    assert report["snapshots"]["count"] == 0


def test_print_dashboard(mock_saguaro_dir, native_compute_stub, capsys):
    dashboard = HealthDashboard(mock_saguaro_dir)
    ...
    assert "Native Compute Fabric" in captured.out
