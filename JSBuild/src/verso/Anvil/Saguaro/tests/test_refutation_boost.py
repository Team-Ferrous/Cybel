...
def test_refutation_boost():
    ...
    print(
        f"No text scores: {results_no_text[0]['name']}={results_no_text[0]['score']:.4f}, {results_no_text[1]['name']}={results_no_text[1]['score']:.4f}"
    )

    # 3. Query WITH query_text targeting "OldClass_v2"
    results_with_text = store.query(
        query_vec, k=2, query_text="Refactor OldClass_v2 implementation"
    )
    print(
        f"With text scores: {results_with_text[0]['name']}={results_with_text[0]['score']:.4f}, {results_with_text[1]['name']}={results_with_text[1]['score']:.4f}"
    )

    # Verify that OldClass_v2 is now boosted (likely top rank)
    assert (
        results_with_text[0]["name"] == "OldClass_v2"
    ), "Refutation boost failed to promote exact match"
