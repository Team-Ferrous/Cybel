// saguaro/native/ops/quantum_embedding_op.h
...
#ifndef SAGUARO_NATIVE_OPS_QUANTUM_EMBEDDING_OP_H_
#define SAGUARO_NATIVE_OPS_QUANTUM_EMBEDDING_OP_H_
...
#include <immintrin.h>
#define SAGUARO_QEMB_AVX512 1
#elif defined(__AVX2__)
#include <immintrin.h>
#define SAGUARO_QEMB_AVX2 1
#elif defined(__ARM_NEON)
#include <arm_neon.h>
#define SAGUARO_QEMB_NEON 1
#else
#define SAGUARO_QEMB_SCALAR 1
#endif
    ...
    for (int b = 0; b < batch_size; ++b) {
        for (int t = 0; t < seq_len; ++t) {
            ...
            for (int bundle = 0; bundle < num_bundles; ++bundle) {
                ...
                for (int d = 0; d < dim; ++d) {
                    T val = bound[d] * inv_bundles;
                    if (!std::isfinite(val)) {
                        // std::cerr << "NaN/Inf detected at b=" << b << " t=" << t << " d=" << d << std::endl;
                        val = 0.0f;
                    } else if (val > 5.0f) {
                        val = 5.0f; 
                    } else if (val < -5.0f) {
...
#endif  // SAGUARO_NATIVE_OPS_QUANTUM_EMBEDDING_OP_H_
