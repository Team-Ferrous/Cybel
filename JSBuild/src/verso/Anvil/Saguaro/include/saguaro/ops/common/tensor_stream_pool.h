 ...
 * 
 * Reads HIGHNOON_TENSOR_STREAM_MAX_GB environment variable for runtime
...
inline size_t GetMaxPoolSizeBytes() {
    ...
    const char* env_val = std::getenv("HIGHNOON_TENSOR_STREAM_MAX_GB");
    if (env_val != nullptr) {
 ...
 * 
 * Reads HIGHNOON_TENSOR_STREAM_WARN_THRESHOLD environment variable.
 * 
 * @return Warning threshold (0.0 to 1.0).
 */
inline float GetWarningThreshold() {
    const char* env_val = std::getenv("HIGHNOON_TENSOR_STREAM_WARN_THRESHOLD");
    if (env_val != nullptr) {
...
public:
    ...
    // -------------------------------------------------------------------------
    // Debug Validation (enabled with HIGHNOON_DEBUG_STREAMING)
    // -------------------------------------------------------------------------
    
#ifdef HIGHNOON_DEBUG_STREAMING
    /**
 ...
 * 
 * Usage:
 *   hsmn::ops::GetTensorStreamPool().Acquire(...);
 *   hsmn::ops::GetTensorStreamPool().Handoff(...);
 *   hsmn::ops::GetTensorStreamPool().Release(...);
 */
...
#define TENSOR_STREAM_ACQUIRE(size_bytes) \
    hsmn::ops::GetTensorStreamPool().Acquire((size_bytes), __func__)

/**
 * @brief Handoff buffer with automatic consumer hint.
 */
#define TENSOR_STREAM_HANDOFF(ptr, consumer) \
    hsmn::ops::GetTensorStreamPool().Handoff((ptr), (consumer))

/**
 * @brief Release buffer.
 */
#define TENSOR_STREAM_RELEASE(ptr) \
    hsmn::ops::GetTensorStreamPool().Release((ptr))
