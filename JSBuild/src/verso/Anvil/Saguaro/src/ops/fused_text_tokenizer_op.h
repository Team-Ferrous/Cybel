 ...
 *
 * MEMORY SAFETY: Uses std::unique_ptr for external_children to ensure proper
 * cleanup and prevent double-free issues during container reallocation.
 */
struct TrieNode {
    ...
    std::unique_ptr<std::unordered_map<int32_t, int32_t>> external_children;
    ...
    // Default destructor is safe because unique_ptr handles cleanup
    ~TrieNode() = default;

    // Disable copy (move only) - unique_ptr is not copyable
    TrieNode(const TrieNode&) = delete;
    TrieNode& operator=(const TrieNode&) = delete;

    // Move constructor - unique_ptr handles ownership transfer
    TrieNode(TrieNode&& other) noexcept
        : inline_count(other.inline_count),
          superword_id(other.superword_id),
          external_children(std::move(other.external_children)) {
    ...
    inline void set_child(int32_t token_id, int32_t node_index) {
        ...
        if (!external_children) {
            external_children = std::make_unique<std::unordered_map<int32_t, int32_t>>();
 ...
 * where N is the n-gram length. Uses std::deque for stable node storage that
 * doesn't invalidate references on insertion.
 *
 * MEMORY SAFETY: Uses std::deque instead of std::vector to prevent
 * iterator/reference invalidation during node insertion. This is critical
 * when nodes reference each other by index during trie construction.
 *
...
public:
    ...
    // Explicit destructor to ensure proper cleanup order
    ~SuperwordTrie() {
        // Clear in reverse order to handle any potential dependencies
        nodes_.clear();
    }

    // Disable copy
    SuperwordTrie(const SuperwordTrie&) = delete;
    SuperwordTrie& operator=(const SuperwordTrie&) = delete;

    // Allow move
    SuperwordTrie(SuperwordTrie&&) = default;
    SuperwordTrie& operator=(SuperwordTrie&&) = default;
    ...
    void insert(const int32_t* ngram, int ngram_len, int32_t superword_id) {
        ...
        for (int i = 0; i < ngram_len; ++i) {
            ...
            if (child_idx < 0) {
                // Create new node - deque guarantees stable references
...
private:
    // Using std::deque for stable references during insertion
    // Unlike std::vector, deque doesn't invalidate references when growing
    std::deque<TrieNode> nodes_;
};
...
// =============================================================================
// COMPLEX TOKENIZATION WITH THINKING TAGS
// =============================================================================

/**
 * @brief Complex tokenization with detection of special thinking tags.
 *
 * Scans for <think>, <pause>, <reflect>, <conclude> and inserts corresponding IDs.
 * Between tags, delegates to SIMD tokenizer.
 */
inline int text_tokenize_utf8_complex(
    const uint8_t* utf8_data,
    int utf8_len,
    int32_t* output_tokens,
    int32_t byte_offset,
    bool add_special_tokens,
    bool inject_thinking
) {
    if (!inject_thinking) {
        return text_tokenize_utf8_simd(utf8_data, utf8_len, output_tokens, byte_offset, add_special_tokens);
    }
    ...
    const char* input_ptr = reinterpret_cast<const char*>(utf8_data);
    int current_idx = 0;

    struct Tag { const char* s; int len; int32_t id; };
    const Tag tags[] = {
        {"<think>", 7, TEXT_TOK_THINK_ID},
        {"<pause>", 7, TEXT_TOK_PAUSE_ID},
        {"<reflect>", 9, TEXT_TOK_REFLECT_ID},
        {"<conclude>", 10, TEXT_TOK_CONCLUDE_ID}
    };

    while (current_idx < utf8_len) {
        // Find earliest tag
        int best_tag_idx = -1;
        int best_pos = utf8_len;

        for (int t = 0; t < 4; ++t) {
            // Simple strstr-like search from current position
            for (int i = current_idx; i <= utf8_len - tags[t].len; ++i) {
                if (i >= best_pos) break; // Optimization
                if (std::memcmp(input_ptr + i, tags[t].s, tags[t].len) == 0) {
                    best_pos = i;
                    best_tag_idx = t;
                    break;
                }
        ...
        if (best_tag_idx != -1) {
            ...
            int segment_len = best_pos - current_idx;
            if (segment_len > 0) {
                // Tokenize segment (no special tokens)
                out_pos += text_tokenize_utf8_simd(
                    utf8_data + current_idx,
                    segment_len,
                    output_tokens + out_pos,
                    byte_offset,
                    false // No CLS/EOS for segments
                );
            }

            // Add tag ID
            output_tokens[out_pos++] = tags[best_tag_idx].id;

            // Advance
            current_idx = best_pos + tags[best_tag_idx].len;
        } else {
            // No more tags, process remaining
            int remaining = utf8_len - current_idx;
            out_pos += text_tokenize_utf8_simd(
                utf8_data + current_idx,
                remaining,
                output_tokens + out_pos,
...
inline int trie_apply_merges(
    ...
    if (trie.empty()) {
        // No superwords, just copy (with truncation)
        int copy_len = std::min(input_len, max_length);
        std::memcpy(output_tokens, input_tokens, copy_len * sizeof(int32_t));
        return copy_len;
    }
    ...
    while (i < input_len && out_pos < max_length) {
        int match_len = 0;
        // Optimization: if remaining space is 1, just take the token if no immediate match or to be safe
        // Actually, find_longest_match checks i < len
...
inline void text_tokenize_batch_parallel(
    ...
    auto process_range = [&](int start, int end) {
        ...
        for (int i = start; i < end; ++i) {
            ...
            // SIMD tokenize to scratch (or complex if thinking enabled)
            int len = text_tokenize_utf8_complex(
                texts[i],
                text_lens[i],
                scratch.data(),
                byte_offset,
            ...
            int final_len = std::min(len, max_length);
            
            if (final_len > 0) {
              std::memcpy(out_ptr, scratch.data(), final_len * sizeof(int32_t));
            }
            
            // Handle explicit EOS if truncated and requested?
            // Existing logic: "If len > max_length ... out_ptr[max_length - 1] = EOS"
...
inline int fused_tokenize_and_merge(
    ...
    if (trie.empty()) {
        int copy_len = std::min(raw_len, max_length);
        std::memcpy(output_tokens, scratch_buffer, copy_len * sizeof(int32_t));
        return copy_len;
    }

    return trie_apply_merges(scratch_buffer, raw_len, trie, output_tokens, max_length);
}
