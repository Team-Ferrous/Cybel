...
void RunLiftingCascadeForward(
    ...
    for (int level = 0; level < num_levels; ++level) {
        ...
        hsmn::parallel::ForShard(static_cast<std::size_t>(batch), 5000, level_work);
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (use_lifting_scheme_) {
      ...
      for (int level = 0; level < num_wavelet_levels_; ++level) {
          ...
          hsmn::parallel::ForShard(static_cast<std::size_t>(batch), 1000, level_work);
