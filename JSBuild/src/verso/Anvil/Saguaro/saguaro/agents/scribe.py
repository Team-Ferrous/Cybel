...
class Scribe:
    ...
    def _init_highnoon(self) -> None:
        """Initialize HighNoon components with fallback."""
        try:
            import highnoon as hn  # noqa: F401
            ...
            if self.hn_model is None:
                logger.info(
                    "Scribe running in HighNoon DRY-RUN mode (no model loaded)."
                )
    ...
    def _tool_submit_patch(
        ...
        return json.dumps(
            {
                "status": "success",
                "patch": {"target_file": target_file, "operations": operations},
            }
        )

    def generate_patch(
        self, task_description: str, context_files: list[str] = None
    ) -> dict[str, Any]:
        ...
        if self.hn_runner:
            ...
            try:
                ...
                def capture_patch(args: dict[str, Any]) -> str:
                ...
                self.hn_runner.manifest.register(
                    ToolRegistration(
                        name="submit_patch",
                ...
                if self.hn_runner.dry_run:
                    ...
                    return {
                        "target_file": (
                            context_files[0] if context_files else "generated.py"
                        ),
                        "operations": [
                            {
                                "op": "insert",
