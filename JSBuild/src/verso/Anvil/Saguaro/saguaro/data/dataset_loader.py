...
class DatasetLoader:
    ...
    def __init__(self, curriculum_path: str | None = None) -> None:
    ...
    def _get_text_from_item(self, ds_name: str, item: dict) -> str | None:
    ...
    def stream_samples(
        self,
        preset_name: str,
        total_limit_bytes: int = 100_000_000,
        max_workers: int = 8,
    ) -> Iterator[str]:
        ...
        for f, s in itertools.zip_longest(fast_streams, slow_streams):
            if f:
                interleaved_streams.append(f)
            if s:
                interleaved_streams.append(s)

        num_streams = len(interleaved_streams)
        logger.info(
            f"Parallel streaming from {num_streams} interleaved streams ({len(hf_datasets)} datasets) with {min(max_workers, num_streams)} workers..."
        )
