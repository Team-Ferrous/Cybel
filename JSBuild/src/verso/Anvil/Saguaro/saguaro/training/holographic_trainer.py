...
class HolographicTrainer:
    ...
    def _init_haar_keys(self, vocab_size: int, dim: int) -> tf.Tensor:
        ...
        return tf.nn.l2_normalize(keys, axis=-1)
    ...
    def train_step(
        ...
        self.optimizer.apply_gradients(zip(gradients, [self.holographic_store], strict=False))
    ...
    def train(
        ...
        eval_data: (
            Iterator[tf.Tensor] | Callable[[], Iterator[tf.Tensor]] | None
        ) = None,
        ...
        for _step, token_ids in enumerate(train_data):
            ...
            if eval_data and self.global_step % self.config.eval_interval == 0:
                ...
                if (
                    ...
                    < self.best_loss - self.config.early_stopping_min_delta
                ):
            ...
            if (
                ...
                ckpt_path = os.path.join(
                    self.config.checkpoint_dir, f"checkpoint_step_{self.global_step}"
                )
                self.save(ckpt_path)
