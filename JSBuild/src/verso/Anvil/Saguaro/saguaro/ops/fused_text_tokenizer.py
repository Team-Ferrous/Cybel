...
def _create_superword_trie_handle() -> tf.Tensor:
    """Create a SuperwordTrie resource handle using the canonical native op."""
    if _text_tok_module is None:
        raise RuntimeError("Native text tokenizer ops not available.")
    create_op = _resolve_trie_create_op(_text_tok_module)
    if create_op is None:
        raise RuntimeError(
            "Loaded SAGUARO core is missing required trie creation op."
        )
    return create_op()
...
class SuperwordTrieHandle:
    ...
    def __init__(self) -> None:
        ...
        if not _ops_available:
            raise RuntimeError("Native text tokenizer ops not available. ")
        self._handle = _create_superword_trie_handle()
        self._num_entries = 0
