...
from saguaro.utils.file_utils import resolve_saguaro_cmd
...
DO NOT use grep_search or find_by_name - use this tool instead."""
    ...
    repo_path: str = "."
    saguaro_cmd: list[str] = None

    def __init__(self, repo_path: str = ".", **kwargs) -> None:
        """Initialize the instance."""
        super().__init__(**kwargs)
        self.repo_path = repo_path
        self.saguaro_cmd = resolve_saguaro_cmd(repo_path)
    ...
    def _run(
        ...
        try:
            result = subprocess.run(
                [*self.saguaro_cmd, "query", query, "--k", str(k), "--json"],
                capture_output=True,
                text=True,
                cwd=self.repo_path,
                timeout=30,
            ...
            return f"Error: Saguaro not found ({self.saguaro_cmd}). Run 'pip install -e .' or 'saguaro init' first."
...
After using this, use saguaro_context_slice to read specific functions."""
    ...
    def _run(
        self, file_path: str, run_manager: CallbackManagerForToolRun | None = None
    ) -> str:
        """Generate file skeleton."""
        try:
            result = subprocess.run(
                [*self.saguaro_cmd, "agent", "skeleton", file_path],
                capture_output=True,
                text=True,
                cwd=self.repo_path,
                timeout=30,
            ...
            return f"Error: Saguaro not found ({self.saguaro_cmd})"
        except Exception as e:
            return f"Error: {str(e)}"
...
If symbol not found, use saguaro_semantic_search to find it."""
    ...
    def _run(
        ...
        try:
            result = subprocess.run(
                [*self.saguaro_cmd, "agent", "slice", symbol, "--depth", str(depth)],
                capture_output=True,
                text=True,
                cwd=self.repo_path,
                timeout=30,
...
If violations found, fix them before marking task complete."""
    ...
    async def _arun(
        self, path: str = ".", engines: str = "native,ruff,semantic,aes"
    ) -> str:
