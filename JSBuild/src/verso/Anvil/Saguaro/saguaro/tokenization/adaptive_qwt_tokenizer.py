...
class AdaptiveQWTTokenizer(QWTTextTokenizer):
    ...
    def learn_from_corpus(
        ...
        def tokenize_text(text: str, **kwargs: Any) -> list[int]:
    ...
    def _encode_one(self, text: str, **kwargs: object):
    ...
    def _pad_result(
        ...
        if single:
            ...
            new_ids = []
            new_masks = []
            for ids, mask in zip(result["input_ids"], result["attention_mask"], strict=False):
                if len(ids) < target_length:
                    pad_amount = target_length - len(ids)
