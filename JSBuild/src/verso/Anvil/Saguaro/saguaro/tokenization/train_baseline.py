...
def train_baseline(
    ...
    if HAS_DATASETS:
        ...
        try:
            ...
            presets_path = (
                base_dir / "saguaro" / "artifacts" / "curriculum_presets.json"
            )
                    ...
                    for ds_name in hf_datasets:
                        try:
                            ...
                            dataset = load_dataset(
                                ds_name, split="train", streaming=True
                            )
    ...
    config = VocabControllerConfig(
        min_ngram_freq=10, sample_size=len(samples), min_ngram_size=2, max_ngram_size=5
    )
    ...
    try:
        ...
        if output_path:
            ...
            target_path = (
                base_dir / "saguaro" / "artifacts" / "codebooks" / "verso_baseline.json"
            )
