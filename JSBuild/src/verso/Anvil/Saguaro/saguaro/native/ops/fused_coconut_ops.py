# saguaro.native/ops/fused_coconut_ops.py
...
Example:
    >>> from saguaro.native.ops.fused_coconut_ops import fused_coconut_bfs
...
def _load_coconut_ops() -> bool:
    ...
    Uses the consolidated binary loading from `saguaro.native`.
    ...
    try:
        from saguaro.native import _load_consolidated_binary
...
def fused_coconut_bfs(
    ...
    if not _load_coconut_ops():
        raise RuntimeError(
            ...
            "Build with: cd saguaro.native && ./build_secure.sh --lite --debug"
        )
