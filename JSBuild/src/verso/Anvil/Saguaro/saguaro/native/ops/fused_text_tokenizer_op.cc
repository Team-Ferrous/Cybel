...
public:
    ...
    void Compute(OpKernelContext* ctx) override {
        ...
        // Use a temp buffer for safety
        std::vector<int32_t> temp_output(utf8_len + 2);
        ...
        if (trie_resource != nullptr) {
            ...
            final_len = text_tokenize_utf8_simd(
                utf8_data,
                utf8_len,
                temp_output.data(),
                byte_offset_,
        ...
        // Copy to output tensor with bounds checking
        int copy_len = std::min(final_len, max_length_);
        std::memcpy(out_ptr, temp_output.data(), copy_len * sizeof(int32_t));
...
REGISTER_KERNEL_BUILDER(Name("FusedTextTokenize").Device(DEVICE_CPU), FusedTextTokenizeOp);

// =============================================================================
// OP REGISTRATION: FusedTextTokenizeBatch
// =============================================================================

REGISTER_OP("FusedTextTokenizeBatch")
    ...
    void Compute(OpKernelContext* ctx) override {
        ...
        if (trie_resource != nullptr && !trie_resource->trie().empty()) {
            ...
            auto process_range = [&](int start, int end) {
                // Initial size guess, but will grow if needed
                std::vector<int32_t> scratch(max_length_ + 2);
                
                for (int i = start; i < end; ++i) {
                    // Ensure scratch buffer is large enough for the full input
                    // potentially generating one token per byte + special tokens
                    size_t required = text_lens[i] + 2;
                    ...
                    // Use a temporary buffer for the output to avoid overflowing the tensor
                    // if the input text length > max_length_
                    std::vector<int32_t> temp_output(required);
                    
                    int final_len = fused_tokenize_and_merge(
                        text_ptrs[i],
                        text_lens[i],
                        trie_resource->trie(),
                        temp_output.data(), // Write to safe temp buffer
                        byte_offset_,
                        add_special_tokens_,
                        scratch.data()
                    );
...
REGISTER_KERNEL_BUILDER(Name("FusedTextTokenizeBatch").Device(DEVICE_CPU), 
...
REGISTER_OP("SuperwordTrieCreate")
...
REGISTER_KERNEL_BUILDER(Name("SuperwordTrieCreate").Device(DEVICE_CPU),
                        SuperwordTrieCreateOp);
...
// =============================================================================
// STREAMING N-GRAM COUNTER RESOURCE
// =============================================================================

class StreamingNgramCounterResource : public ResourceBase {
public:
    StreamingNgramCounterResource(int min_ngram, int max_ngram)
        : counter_(min_ngram, max_ngram) {}

    StreamingNgramCounter& counter() { return counter_; }
    const StreamingNgramCounter& counter() const { return counter_; }

    string DebugString() const override {
        return strings::StrCat("StreamingNgramCounterResource with ", 
                              counter_.unique_count(), " unique n-grams");
    }

private:
    StreamingNgramCounter counter_;
};
...
REGISTER_OP("StreamingNgramCountCreate")
    ...
    .Attr("shared_name: string = ''")
    .SetShapeFn(shape_inference::ScalarShape)
    .Doc("Create a StreamingNgramCounter resource.");

class StreamingNgramCountCreateOp : public ResourceOpKernel<StreamingNgramCounterResource> {
public:
    explicit StreamingNgramCountCreateOp(OpKernelConstruction* ctx)
        : ResourceOpKernel<StreamingNgramCounterResource>(ctx) {
        OP_REQUIRES_OK(ctx, ctx->GetAttr("min_ngram", &min_ngram_));
        OP_REQUIRES_OK(ctx, ctx->GetAttr("max_ngram", &max_ngram_));
    }

private:
    Status CreateResource(StreamingNgramCounterResource** resource) override {
        *resource = new StreamingNgramCounterResource(min_ngram_, max_ngram_);
        return Status();
    }
...
REGISTER_KERNEL_BUILDER(Name("StreamingNgramCountCreate").Device(DEVICE_CPU),
                        StreamingNgramCountCreateOp);
...
handle: Handle to StreamingNgramCounterResource.
tokens: 1D flat int32 tensor of concatenated token sequences.
lengths: 1D int32 tensor of sequence lengths.
)doc");
...
public:
    ...
    void Compute(OpKernelContext* ctx) override {
        StreamingNgramCounterResource* resource = nullptr;
        OP_REQUIRES_OK(ctx, LookupResource(ctx, HandleFromInput(ctx, 0), &resource));
        core::ScopedUnref unref(resource);

        const Tensor& tokens_tensor = ctx->input(1);
        const Tensor& lengths_tensor = ctx->input(2);

        auto tokens_flat = tokens_tensor.flat<int32_t>();
        auto lengths_flat = lengths_tensor.flat<int32_t>();
        int num_sequences = lengths_tensor.dim_size(0);

        const int32_t* tokens_ptr = tokens_flat.data();
        
        // Multi-threaded counting if batch is large enough
        // Note: StreamingNgramCounter::count_sequence is thread-safe
        int num_threads = std::max(1, static_cast<int>(std::thread::hardware_concurrency()));
        
        if (num_sequences <= num_threads * 10) {
            // Process sequentially
            int offset = 0;
            for (int i = 0; i < num_sequences; ++i) {
                int len = lengths_flat(i);
                resource->counter().count_sequence(tokens_ptr + offset, len);
                offset += len;
            }
        } else {
            // Calculate offsets completely first
            std::vector<int> offsets(num_sequences + 1, 0);
            for (int i = 0; i < num_sequences; ++i) {
                offsets[i+1] = offsets[i] + lengths_flat(i);
            }

            auto process_range = [&](int start, int end) {
                for (int i = start; i < end; ++i) {
                    resource->counter().count_sequence(
                        tokens_ptr + offsets[i], 
                        lengths_flat(i)
                    );
                }
            };
            ...
            std::vector<std::thread> threads;
            int chunk = (num_sequences + num_threads - 1) / num_threads;
            for (int t = 0; t < num_threads; ++t) {
                int start = t * chunk;
                int end = std::min(start + chunk, num_sequences);
                if (start < end) {
                    threads.emplace_back(process_range, start, end);
                }
            }
...
REGISTER_KERNEL_BUILDER(Name("StreamingNgramCount").Device(DEVICE_CPU),
                        StreamingNgramCountOp);
...
REGISTER_OP("StreamingNgramCountExport")
    ...
    .Output("counts: int32")
    .Output("splits: int32")
    .Attr("min_frequency: int = 1")
    .Attr("max_count: int = 1000")
    .SetShapeFn([](shape_inference::InferenceContext* c) {
    ...
    .Doc(R"doc(
Export top-K frequent n-grams from the counter.

handle: Handle to StreamingNgramCounterResource.
min_frequency: Minimum occurrence count.
max_count: Maximum number of n-grams to return.
ngrams: Flat tensor of n-gram tokens.
counts: Frequency counts.
splits: Split indices for ngrams (like RaggedTensor).
)doc");
...
public:
    explicit StreamingNgramCountExportOp(OpKernelConstruction* ctx) : OpKernel(ctx) {
        OP_REQUIRES_OK(ctx, ctx->GetAttr("min_frequency", &min_frequency_));
    ...
    void Compute(OpKernelContext* ctx) override {
        ...
        resource->counter().get_top_ngrams(min_frequency_, max_count_, top_ngrams);

        // Flatten for Tensors
        std::vector<int32_t> ngrams_flat;
        std::vector<int32_t> counts_vec;
        std::vector<int32_t> splits_vec;
        
        splits_vec.push_back(0);
        int current_offset = 0;

        for (const auto& pair : top_ngrams) {
            ngrams_flat.insert(ngrams_flat.end(), pair.first.begin(), pair.first.end());
            counts_vec.push_back(pair.second);
            
            current_offset += pair.first.size();
            splits_vec.push_back(current_offset);
        }

        // Allocate outputs
        Tensor* ngrams_tensor = nullptr;
        OP_REQUIRES_OK(ctx, ctx->allocate_output(0, 
            TensorShape({static_cast<int64_t>(ngrams_flat.size())}), &ngrams_tensor));
        
        Tensor* counts_tensor = nullptr;
        OP_REQUIRES_OK(ctx, ctx->allocate_output(1, 
            TensorShape({static_cast<int64_t>(counts_vec.size())}), &counts_tensor));
        
        Tensor* splits_tensor = nullptr;
        OP_REQUIRES_OK(ctx, ctx->allocate_output(2, 
            TensorShape({static_cast<int64_t>(splits_vec.size())}), &splits_tensor));

        // Copy data
        std::memcpy(ngrams_tensor->flat<int32_t>().data(), ngrams_flat.data(), 
                   ngrams_flat.size() * sizeof(int32_t));
        std::memcpy(counts_tensor->flat<int32_t>().data(), counts_vec.data(), 
                   counts_vec.size() * sizeof(int32_t));
        std::memcpy(splits_tensor->flat<int32_t>().data(), splits_vec.data(), 
                   splits_vec.size() * sizeof(int32_t));
    }
...
REGISTER_KERNEL_BUILDER(Name("StreamingNgramCountExport").Device(DEVICE_CPU),
                        StreamingNgramCountExportOp);
