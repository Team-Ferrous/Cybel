// saguaro.native/ops/quantum_lm_head_hd_op.h
...
#ifndef SAGUARO_NATIVE_OPS_QUANTUM_LM_HEAD_HD_OP_H_
#define SAGUARO_NATIVE_OPS_QUANTUM_LM_HEAD_HD_OP_H_
...
inline void QuantumLMHeadHDForward(
    ...
    for (int b = 0; b < batch_size; ++b) {
        for (int s = 0; s < seq_len; ++s) {
            ...
            std::vector<float> state_re = projected;
            std::vector<float> state_im(vqc_dim, 0.0f);
            ...
            for (int layer = 0; layer < num_layers; ++layer) {
                ...
                for (int q = 0; q < num_qubits; ++q) {
                    ...
                    if (layer == 0 && q == config.entropy_target_qubit) {
                        float entropy_phase = entropy * config.entropy_scale * M_PI;
                        rz_angle += entropy_phase;
                    }

                    apply_ry_inplace(state_re.data(), state_im.data(),
                                     vqc_dim, q, num_qubits, ry_angle);
                    apply_rz_inplace(state_re.data(), state_im.data(),
                ...
                for (int q = 0; q < num_qubits - 1; ++q) {
                    ...
                    if (std::abs(strength) > 0.5f) {
                        apply_cnot_inplace(state_re.data(), state_im.data(),
                                           vqc_dim, q, q + 1, num_qubits);
                    }
                }
            }

            // 5. Compute probabilities via Born rule
            std::vector<float> probs(vqc_dim);
...
#endif  // SAGUARO_NATIVE_OPS_QUANTUM_LM_HEAD_HD_OP_H_
