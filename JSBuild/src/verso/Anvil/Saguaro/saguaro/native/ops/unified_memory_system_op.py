...
class MemoryConfig:
    ...
    def validate(self) -> bool:
        ...
        if self.mem_type not in MemoryType:
            return False
        return not (self.num_slots < 1 or self.slot_dim < 1)
...
class UnifiedMemoryLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: MemoryConfig, name: str | None = None, **kwargs) -> None:
        super().__init__(
            name=name or f"memory_{config.mem_type.name.lower()}", **kwargs
        )
        self.config = config
    ...
    def call(self, query, training=None):
        ...
        output, attention = unified_memory_read(
            query, self.memory, self.config, aux_data
        )
        return output
