# saguaro.native/ops/hd_streaming_adapter.py
...
Usage:
    >>> from saguaro.native.ops.hd_streaming_adapter import HDStreamingAdapter
...
def _load_hd_streaming_ops() -> None:
    ...
    from saguaro.native import resolve_op_library
    ...
    if os.path.exists(consolidated_lib_path):
        try:
            ...
            hd_streaming_project_op = getattr(
                ...
                getattr(_hd_streaming_module, "HDStreamingProject", None),
            )
            hd_streaming_project_grad_op = getattr(
                _hd_streaming_module,
                "hd_streaming_project_grad",
                getattr(_hd_streaming_module, "HDStreamingProjectGrad", None),
            )
            if hd_streaming_project_op is not None:
                logger.info(
                    "Loaded HDStreamingProject from consolidated _saguaro_core.so"
                )
                return
...
def hd_streaming_project(
    ...
    if hd_streaming_project_op is None:
        raise NotImplementedError(
            ...
            "Please compile it: ./saguaro.native/build_secure.sh"
        )
