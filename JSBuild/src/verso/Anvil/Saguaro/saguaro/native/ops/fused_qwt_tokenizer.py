# saguaro.native/ops/fused_qwt_tokenizer.py
...
try:
    from saguaro.native import fused_qwt_tokenizer_op_path, load_qwt_tokenizer

    _qwt_module = load_qwt_tokenizer()
    if _qwt_module is not None:
        _fused_qwt_tokenizer = getattr(
            _qwt_module,
            "fused_qwt_tokenizer",
            getattr(_qwt_module, "FusedQwtTokenizer", None),
        )
        _fused_qwt_tokenizer_grad = getattr(
            _qwt_module,
            "fused_qwt_tokenizer_grad",
            getattr(_qwt_module, "FusedQwtTokenizerGrad", None),
        )
        if _fused_qwt_tokenizer is not None:
            _OP_LIBRARY_PATH = fused_qwt_tokenizer_op_path()
            logger.info("Loaded fused_qwt_tokenizer op from consolidated binary")
        else:
            logger.warning(
                "fused_qwt_tokenizer symbol not found in loaded native module; "
                "QWT tokenizer op disabled."
            )
    else:
        logger.error("fused_qwt_tokenizer op not found in consolidated binary")
except (ImportError, AttributeError) as err:  # pragma: no cover - load-time
