# saguaro.native/ops/lib_loader.py
...
from saguaro.native.runtime.arch import (
...
def get_consolidated_library(target_arch: str | None = None) -> str | None:
    ...
    if override:
        ...
        logger.warning("SAGUARO_CORE_BINARY points to missing file: %s", override)

    # Prefer canonical in-tree consolidated binary first.
    in_tree_path = _NATIVE_DIR.parent / "_saguaro_core.so"
    if in_tree_path.exists():
        return str(in_tree_path)
