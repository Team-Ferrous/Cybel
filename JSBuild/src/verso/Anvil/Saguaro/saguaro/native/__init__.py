...
def _find_core_binary() -> Path | None:
    ...
    override = os.getenv("SAGUARO_CORE_BINARY")
    if override:
        override_path = Path(override)
        if override_path.exists():
            return override_path
        log.warning("SAGUARO_CORE_BINARY points to missing file: %s", override)

    # 2. Check saguaro directory (canonical in-tree consolidated binary)
    saguaro_root_path = _NATIVE_DIR.parent / _CORE_BINARY_NAME
    if saguaro_root_path.exists():
        return saguaro_root_path

    # 3. Check standard bin directory
    core_path = _BIN_DIR / _CORE_BINARY_NAME
    if core_path.exists():
        return core_path

    # 4. Check build directory (developer fallback)
...
def resolve_op_library(caller_file: str, library_name: str) -> str:
    ...
    """
    # Keep consolidated core resolution in sync with _find_core_binary to
    # avoid loading stale per-arch binaries when a newer in-tree core exists.
    if library_name == _CORE_BINARY_NAME:
        core_path = _find_core_binary()
        if core_path is not None:
            return str(core_path)

    # Return path in the platform-specific bin directory for legacy per-op libs.
...
def check_enterprise_license(domain: str) -> bool:
    ...
    """
    from saguaro.native._limits import check_enterprise_license as _check
