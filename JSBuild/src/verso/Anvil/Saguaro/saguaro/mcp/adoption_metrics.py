...
class SessionMetrics:
    ...
    def total_fallback(self) -> int:
        """Handle total fallback."""
        return (
            self.grep_search + self.find_by_name + self.view_file + self.view_code_item
        )
...
class AdoptionTracker:
    ...
    SAGUARO_TOOLS = {
        "saguaro_query",
        "saguaro_semantic_search",
        "query",
        "saguaro_skeleton",
        "saguaro_file_skeleton",
        "skeleton",
        "saguaro_slice",
        "saguaro_context_slice",
        "slice",
        "saguaro_verify",
        "verify",
    }
    ...
    def __init__(self, saguaro_dir: str | None = None) -> None:
    ...
    def record_tool_use(self, tool_name: str, context: str | None = None) -> None:
    ...
    def record_fallback_use(self, tool_name: str, context: str | None = None) -> None:
    ...
    def _record(self, tool_name: str, is_saguaro: bool, context: str | None = None) -> None:
            ...
            "current_session": (
                {
                    ...
                    "score": (
                        self.current_session.adoption_score
                        if self.current_session
                        else None
                    ),
                    "events": (
                        len(self.current_session.events) if self.current_session else 0
                    ),
                }
                if self.current_session
                else None
...
_global_tracker: AdoptionTracker | None = None
