...
class SaguaroToolInterceptor:
    ...
    def intercept(
        self, tool_name: str, args: dict, context: dict | None = None
    ) -> InterceptResult:
        ...
        if exception_reason in self.ALLOWED_EXCEPTIONS:
            logger.debug(
                f"Tool {tool_name} allowed due to exception: {exception_reason}"
            )
...
"""
    ...
    def _generate_suggestion(self, tool_name: str, args: dict) -> str:
        ...
        if tool_name in ("grep_search", "find_by_name"):
            ...
            return f'saguaro query "{query}" --k 5'

        if tool_name == "view_file":
            file_path = args.get("path") or args.get("AbsolutePath") or "file.py"
            return f"""saguaro agent skeleton {file_path}
# Then to read a specific symbol:
saguaro agent slice SymbolName --depth 2"""

        if tool_name == "view_code_item":
            symbol = (
                args.get("symbol") or args.get("NodePaths", ["Symbol"])[0] or "Symbol"
            )
            return f"saguaro agent slice {symbol} --depth 2"

        return 'saguaro query "description" --k 5'
...
def create_langchain_tool_wrapper(
    interceptor: SaguaroToolInterceptor,
