...
class SafetyEngine:
    ...
    def safe_delete(
        self, target_path: str, force: bool = False, dry_run: bool = True
    ) -> dict[str, Any]:
        ...
        dependents = []
        if "tests_impacted" in report:
            dependents.extend(report["tests_impacted"])
        if "interfaces_impacted" in report:
            dependents.extend(report["interfaces_impacted"])
        ...
        dest = os.path.join(
            trash_dir, f"{os.path.basename(abs_path)}_{int(time.time())}"
        )
